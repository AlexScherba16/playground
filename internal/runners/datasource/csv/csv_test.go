...
func getResult(
	...
	ds, err := NewDataSource(ctx, wg, path, rCh, eCh)
...
func TestNewDataSource_InvalidInputParams(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(testCase.name, func(t *testing.T) {
			...
			/* ACT */
			result, err := NewDataSource(testCase.input.ctx, testCase.input.wg, testCase.input.path, testCase.input.rCh, testCase.input.eCh)
			...
			if (err != nil) && (err.Error() != errorStr) {
				t.Fatalf("NewDataSource() : expected error string [%s], got [%s]", errorStr, err.Error())
			}
...
func TestCsvDataSource_RunInvalidFileOpening(t *testing.T) {
	/* ARRANGE */
...
func TestNewDataSource_RunCancelReadingCsvFile(t *testing.T) {
	...
	source, _ := NewDataSource(ctx, in.wg, in.path, in.rCh, in.eCh)
	// Invoke cancel
	cancel()
...
func TestNewDataSource_RunReadCorruptedCsvFileData(t *testing.T) {
	/* ARRANGE */
