...
func TestNewAggregator_InvalidInputParams(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: inputParameters{&s.WaitGroup{}, InvalidCampaignAggregateParameter, tp.NewRecordChannel(0), tp.NewAggregatorChannel(0)},
		...
		},
		{
			...
			input:          inputParameters{&s.WaitGroup{}, cnst.AggregateCampaign, nil, tp.NewAggregatorChannel(0)},
